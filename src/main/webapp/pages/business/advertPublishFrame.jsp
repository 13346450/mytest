...
<script type="text/javascript">
	...
	function clickRow(rowIndex, rowData) {
     ...
     function publish(){
         ...
         if(!row){
         	$.messager.alert('提示信息',"请选中需要发布的广告。",'error');
         	return;
         }
 		$.messager.confirm('确认','确定要发布选中的广告吗?',function(r){
 		    if (!r){return;}
 	    	var json = {
 	    		    url: "${base}/business/BizAdvert/publishAdvert/" + row.idKey,
     ...
     function cancelPublish(){
         var row = $('#dataList').datagrid('getSelected');
         if(!row){
         	$.messager.alert('提示信息',"请选中需要取消发布的广告。",'error');
         	return;
         }
 		$.messager.confirm('确认','确定要取消发布选中的广告吗?',function(r){
 		    if (!r){return;}
 	    	var json = {
 	    		    url: "${base}/business/BizAdvert/cancelPublishAdvert/" + row.idKey,
 	    		    success: function(result){
 		     	    	if (result.successMsg){
 		     	    		showMask(result.successMsg);
 		     	    		rownum = 0;
 	...
 	function view(){
         ...
         if (row){
         	...
         	$.messager.alert('提示信息',"请选中要发布的广告",'error');
         }
 	}
 	//设置表单是否可编辑
