...
<script type="text/javascript">
	...
	function clickRow(rowIndex, rowData) {
     ...
     function audit(){
         ...
         if(!row){
         	$.messager.alert('提示信息',"请选中需要审核的app版本。",'error');
         	return;
         }
 		$.messager.confirm('确认','确定要审核选中的版本吗?',function(r){
 		    if (!r){return;}
 	    	var json = {
 	    		    url: "${base}/business/BizApp/auditApp/" + row.idKey,
     ...
     function cancelAudit(){
         var row = $('#dataList').datagrid('getSelected');
         if(!row){
         	$.messager.alert('提示信息',"请选中需要取消审核的版本。",'error');
         	return;
         }
 		$.messager.confirm('确认','确定要取消审核选中的版本吗?',function(r){
 		    if (!r){return;}
 	    	var json = {
 	    		    url: "${base}/business/BizApp/cancelAuditApp/" + row.idKey,
 	    		    success: function(result){
 		     	    	if (result.successMsg){
 		     	    		showMask(result.successMsg);
 		     	    		rownum = 0;
