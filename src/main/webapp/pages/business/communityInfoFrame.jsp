...
<script type="text/javascript">
	...
	$(function() {
		$('#dataList').datagrid({
			url : '${base}/system/SysCommunityInfo/listPage',
			...
			columns : [[
			            {field:'displayName', title:'名称', align:'left', width:120},
			            {field:'typeNames', title:'类型', align:'center', width:85},
			            {field:'communityName', title:'所属小区', align:'left', width:100},
			            {field:'summary', title:'简介', align:'left', width:300},
			            {field:'address', title:'地址',align:'center', width:85},
			            {field:'phoneNum', title:'座机号码', align:'center', width:85, sortable:true},
			            {field:'tel', title:'手机号码',align:'center', width:85},
			            {field:'contact', title:'联系人', align:'center', width:85, sortable:true},
			            {field:'url', title:'链接地址',  align:'left',   width:70, sortable:true},
			            {field:'logoUrl', title:'链接地址',  align:'left', hidden:true}
			          ]]
		});
		mainPageView();
		setInitDate();
	...
	{
		//rownum = rowIndex;
		$('#logoImg').attr("src", "${base}/" + rowData.logoUrl);
		edit();
	}
	 /*表格加载成功后调用的方法 */
     ...
     function deleteOne(){
 		...
 		$.messager.confirm('确认','确定要删除选中的信息吗?',function(r){
 		    if (!r){return;}
 	    	var json = {
 	    		    url: "${base}/system/SysCommunityInfo/delete/" + row.idKey,
   ...
   //删除多条
 	function deleteMulti() {
 		...
 		if(checkedItems.length==0){
 			$.messager.alert('提示信息', "请选中需要删除的 信息。", 'error');
 			return;
 		}
 		...
 		$.messager
 				.confirm(
 						'确认',
 						'确定要删除选中的 ' + checkedItems.length + ' 个 信息吗?',
 						function(r) {
 							if (!r)
 								return;
 							var json = {
 								url : "${base}/system/SysCommunityInfo/deleteMulti",
 								formId : "fm1",
	...
	/* 新增一条记录 */
 	function add(){
        ...
        $('#fm').form('clear');
        $('#logoImg').attr("src", "");
		postUrl = "${base}/system/SysCommunityInfo/insert";
 	}
 	/* 编辑一条记录 */
 	function edit()
 	{
 		var row = $('#dataList').datagrid('getSelected');
 		$('#logoImg').attr("src", "${base}/" + row.logoUrl);
        if (row){
        	$('#dlg').dialog('open').dialog('setTitle', '编辑信息');
			$('#fm').form('load', row);
            postUrl = '${base}/system/SysCommunityInfo/update';
        } else {
			$.messager.alert('提示信息', "一次编辑只能选择一条 信息", 'error');
		}
 	}
 	/* 更改输入的格式设置 */
 	function change()
	...
	function save() {
		...
		//cxl start
		var displayName = $("#fm").find("[name='displayName']").val();
		displayName = $.trim(displayName);
		if(displayName==""||displayName.length>50){
			alert("名称不能为空且在50个字符以内");
			return;
		}
		if(!$("#fm").find("[name='tel']").val().match(/[0-9-]{3,20}/)){
			alert("手机号码应为3-20位的数字组成");
			return;
		}
		//cxl end
		change();
	...
	function uploadImage(){
		...
		var json = {
			url : "${base}/system/SysCommunityInfo/uploadFile",
			...
			success : function(result) {
				...
				if (res.successMsg) {
					...
					setTimeout(function() {
						hideMask();
						$('#dl').dialog('close');
						$('#logoUrl').val(res.addr);
						if ($('#logoUrl').val()) {
							$('#logoImg').attr("src", "${base}/" + $('#logoUrl').val());
						} else {
							$('#logoImg').attr("src", "");
						}
					}, DEFAULT_DELAY_MS);
	...
	function changImages() {
		$('#logoUrl').attr("src", "${base}/" + $('#logoUrl').val());
	}
	//显示tip
	...
	function removeImage(obj,image_addr,image_link){
		$.messager.confirm('确认','删除图片后无法恢复，确定吗?',function(r){
 		    if (!r){return;}
			var json = {
	    		    url: "${base}/system/SysCommunityInfo/deleteImage?imageAddr="+image_addr,
	    		    success: function(result){
		     	    	if (result.successMsg){
		     	    		showMask(result.successMsg);
...
<body>
	<div id="bodyLayout" class="easyui-layout" data-options="fit:true">
				...
				<form id="queryForm" method="post">
					<table>
						...
						<tr>
							...
							<td>
								<select class="easyui-combobox" name="status"  data-options="panelHeight:'auto'" style="width:80px">
									<option value="-1">全选</option>
									<option value="1">物业公司</option>
									<option value="2">小区信息</option>
								</select>
							</td>
		...
		<div id="dlg" class="easyui-dialog"  data-options="modal:true,closed:true,iconCls:'icon-edit'" 
	      ...
	      <div class="ftitle">物业客服信息</div>
	    	<form id="fm" method="post" novalidate>		    		
	    		<div class="fitem">
	    				<label>名称:</label>	    				
	    					<input name="idKey" type="hidden">
	    					<input id="logoUrl" name="logoUrl" type="hidden"/>
	    					<input name="displayName" class="easyui-validatebox" data-options="required:true"/>
	    		</div>	    			
	    		<div class="fitem">
	    				<label>小区信息:</label>		    				
	    				  <tag:popedomCommunitySelect
								id="communityId" width="120" name="communityId"
								editable="true" required="true"></tag:popedomCommunitySelect>	
				</div>
	    		<div class="fitem">		
	    				<label>信息类型:</label>
	    				<select class="easyui-combobox" name="type"  data-options="required:true,panelHeight:'auto'" style="width:150px">
									<option value="1">物业公司信息</option>
									<option value="2">小区信息</option>
						</select>
	    		</div>	
	    		<div class="fitem">		
	    		...
	    		<div class="fitem">		
	    				<label>座机:</label>
	    				<input name="phoneNum" class="easyui-validatebox" data-options="required:true"/>
	    		</div>	
	    		<div class="fitem">		
	    				<label>手机号码:</label>
	    				<input name="tel" class="easyui-validatebox" data-options="required:true"/>
	    		</div>	
	    		<div class="fitem">		
	    				<label>备注信息:</label>
	    				<textarea name="summary" rows="5%" id="infoContent" cols="22%" data-options="required:true">
	    				</textarea>
	    		</div>
	    		<div class="fitem">		
	    				<label>联系人:</label>
	    				<input name="contact" class="easyui-validatebox" data-options="required:true"/>
	    		</div>	
	    		<div class="fitem">		
	    				<label>网站地址:</label>
	    				<input name="url" class="easyui-validatebox" data-options="required:true"/>
	    		</div>	
	    		<div class="fitem">	
					<label>logo图片:</label>
					<img id="logoImg" src="" width='200px' height="160px">
