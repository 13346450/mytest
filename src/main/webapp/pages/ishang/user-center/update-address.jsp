...
<div class="container" style="margin-top:-40px; min-height:743px;">
  <div class="row">
     <jsp:include page="common/personalLeftNav.jsp"/>
      ...
      <input name="cmd" value="4401" type="hidden">
      <input name="isDefault" value="0" type="hidden">
      <div id="information" class="marginl20 margint10">
        <p><i class="redstart">*</i>收货人:</p>
          <input type="text" name="userName" maxlength="100" class="border-ccc p08input"/>
        <p><i class="redstart">*</i>联系电话:</p>
          <input type="text" name="phone" maxlength="100" class="border-ccc p08input"/>
        ...
        <input type="text" name="addrDetail" maxlength="100" class="border-ccc p08input" style="width:500px;"/>
        <div style="margin:20px 0 20px 15px;">
          ...
          <i  class="isDefault-i cart-ico cart-ico-input" style="margin:5px 5px 0 80px;"></i>设置为默认收货地址</span>
        <div class="clear"></div>
        </div>
      </div>
      </form>
    ...
    <c:forEach items="${list}" var="item">
	    <li class="border-ccc" style="
	    	<c:if test="${item.isDefault==1}">background:url(${base}/resources/images/ishangimg/firstadd.png) no-repeat left top;</c:if>
	    	">
	     <i data-id="${item.idKey}" class="deleteAddress history-listitempic" style=""></i>
	     <p style="margin:30px 0 10px 62px;">收货人：<span>${item.userName}</span></p>
	     <p class="">手机号码：<span>${item.phoneNum}</span></p>
	     <p class="">所在地区：<span>${item.provinceNm} ${item.cityNm} ${item.districtNm}</span></p>
	     <p class=" ">详细地址：<span>12345678912</span></p>
	     <p style="text-align:right; margin:5px 50px 10px 50px; color:#00a0e9;">
	     	<c:if test="${item.isDefault!=1}">
	     		<span data-id="${item.idKey}" class="setToDefault">设置为默认收货地址</span>
	     	</c:if>
	     	<span class="marginl30" onclick="location.href='${base}/user-center/update-address-2?id=${item.idKey}'">编辑</span>
	     </p>
	     </li>
    </c:forEach>
    </ul>
