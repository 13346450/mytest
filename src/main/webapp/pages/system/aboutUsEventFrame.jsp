...
<script type="text/javascript">
	...
	$(function() {
		$('#dataList').datagrid({
			url : '${base}/system/SysAboutUSEvent/listPage',
			...
			columns : [[
			            {field:'eventTitle', title:'主标题', width:120,align:'left'},
		 	            {field:'eventSubtitle', title:'副标题', width:200, align:'left'},
			            {field:'eventContent', title:'正文', width:300, align:'center'},
			            {field:'happenTime', title:'时间', width:300, align:'center'}
			          ]] 
		});
	...
	function dblClickRow(rowIndex, rowData) {
     ...
     function deleteOne(){
         ...
         if(!row){
         	$.messager.alert('提示信息',"请选中需要删除的事件。",'error');
         	return;
         }
 		$.messager.confirm('确认','确定要删除选中的事件吗?',function(r){
 		    if (!r){return;}
 	    	var json = {
 	    		    url: "${base}/system/SysAboutUSEvent/delete/" + row.idKey,
	...
	/* 新增一条记录 */
 	function add(){
 		$('#dlg').dialog('open').dialog('setTitle','增加事件');
        $('#fm').form('clear');
        postUrl = "${base}/system/SysAboutUSEvent/insert";
 	}
	...
	function queryById()
	{
		...
		$.messager.confirm('确认','确定要删除选中的事件吗?',function(r){
 		    if (!r){return;}
		var json = {
	    		    url: "${base}/system/SysAboutUSEvent/queryById/" + row ,
	    		    success: function(result){
 	...
 	function edit(){
 		var row = $('#dataList').datagrid('getSelected');
         if (row){
             $('#dlg').dialog('open').dialog('setTitle','编辑事件');
             $('#fm').form('load',row);
             postUrl = '${base}/system/SysAboutUSEvent/update';
         }else{
         	$.messager.alert('提示信息',"请选中要编辑的事件",'error');
         }
 	}
 	/* 保存数据 */
	function save() {
	...
	function uploadImage(){
		...
		var json = {
	 		    url: "${base}/system/SysAboutUSEvent/uploadFile",
...
/*  	    var newRow = "<tr>\n"
	...
	function removeImage(obj,image_addr,image_link){
		$.messager.confirm('确认','删除图片后无法恢复，确定吗?',function(r){
 		    if (!r){return;}
			var json = {
	    		    url: "${base}/system/SysAboutUSEvent/deleteImage?imageAddr="+image_addr,
	    		    success: function(result){
		     	    	if (result.successMsg){
		     	    		showMask(result.successMsg);
	...
	<!-- 上部按钮层 -->
		...
		<div id="dlg" class="easyui-dialog"  data-options="modal:true,closed:true,iconCls:'icon-edit'" 
	        ...
	        <form id="fm" method="post" novalidate>
	            ...
	            <div class="fitem">
	            	<label>主标题:</label>
					<input name="eventTitle" class="easyui-validatebox" required="true">
					<label>副标题:</label>
					<input name="eventSubtitle" class="easyui-validatebox"  required="true">
					<label>正文:</label>
					<input name="eventContent" class="easyui-validatebox" required="true">
					<label>时间:</label>
					<input name="happenTime" class="easyui-datebox" required="true">
				</div>
<!-- 				<div class="fitem"> -->
