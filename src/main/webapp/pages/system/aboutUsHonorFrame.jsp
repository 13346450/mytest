...
<script type="text/javascript">
	...
	$(function() {
		$('#dataList').datagrid({
			url : '${base}/system/SysAboutUSHonor/listPage',
			...
			onDblClickRow : dblClickRow,
	 		...
	 		frozenColumns:[[
		 	             ...
		 	             //{field:'id_key', title:'荣誉记录序号',  width:60,  halign:'center', sortable:true}
		 	           ]],
			columns : [[
			            {field:'honorName', title:'荣誉', width:120,align:'left'},
		 	            {field:'honorDate', title:'颁发事件', width:200, align:'left'},
			            {field:'honorBigImageUrl', title:'大图地址', width:300, align:'center'},
			            {field:'honorSmallImageUrl', title:'小图地址', width:300, align:'center'},
			            {field:'awardOrg', title:'颁发组织', width:120, align:'right'},
	...
	function dblClickRow(rowIndex, rowData) {
     ...
     function deleteOne(){
         ...
         if(!row){
         	$.messager.alert('提示信息',"请选中需要删除的荣誉记录。",'error');
         	return;
         }
 		$.messager.confirm('确认','确定要删除选中的荣誉记录吗?',function(r){
 		    if (!r){return;}
 	    	var json = {
 	    		    url: "${base}/system/SysAboutUSHonor/delete/" + row.idKey,
	...
	/* 新增一条记录 */
 	function add(){
 		$('#dlg').dialog('open').dialog('setTitle','增加荣誉记录');
        $('#fm').form('clear');
        postUrl = "${base}/system/SysAboutUSHonor/insert";
 	}
	...
	function queryById()
	{
		...
		$.messager.confirm('确认','确定要删除选中的荣誉记录吗?',function(r){
 		    if (!r){return;}
		var json = {
	    		    url: "${base}/system/SysAboutUSHonor/queryById/" + row ,
	    		    success: function(result){
 	...
 	function edit(){
 		var row = $('#dataList').datagrid('getSelected');
         if (row){
             $('#dlg').dialog('open').dialog('setTitle','编辑荣誉记录');
             $('#fm').form('load',row);
             postUrl = '${base}/system/SysAboutUSHonor/update';
         }else{
         	$.messager.alert('提示信息',"请选中要编辑的荣誉记录",'error');
         }
 	}
 	/* 保存数据 */
	function save() {
	...
	function uploadImage(){
		...
		var json = {
	 		    url: "${base}/system/SysAboutUSHonor/uploadFile",
	 		    ...
	 		    success: function(result){
	 		    	 var res = eval ("(" + result + ")");
		     	      if (res.successMsg){
	     	    		...
	     	    		$('#honorBigImageUrl').val(res.addr);
	     	    		$('#honorSmallImageUrl').val(res.addr);
...
/*  	    var newRow = "<tr>\n"
	...
	function removeImage(obj,image_addr,image_link){
		$.messager.confirm('确认','删除图片后无法恢复，确定吗?',function(r){
 		    if (!r){return;}
			var json = {
	    		    url: "${base}/system/SysAboutUSHonor/deleteImage?imageAddr="+image_addr,
	    		    success: function(result){
		     	    	if (result.successMsg){
		     	    		showMask(result.successMsg);
...
<body>
	...
	<!-- 上部按钮层 -->
		...
		<!-- 下部列表层 -->
		<div data-options="region:'center',title:'&nbsp;荣誉记录列表'">
		...
		<div id="dlg" class="easyui-dialog"  data-options="modal:true,closed:true,iconCls:'icon-edit'" 
	        ...
	        <form id="fm" method="post" novalidate>
	            ...
	            <div class="fitem">
	            	<label>荣誉:</label>
					<input name="honorName" class="easyui-validatebox" required="true">
					<label>颁发时间:</label>
					<input name="honorDate" class="easyui-datebox"  required="true">
					<label>颁发机构:</label>
					<input name="awardOrg" class="easyui-validatebox" required="true">
					<label>备注:</label>
					<input name="remark" class="easyui-validatebox" required="true">
					<label>大图:</label>
					<input id="honorBigImageUrl" name="honorBigImageUrl" required="true" type="text" readonly="readonly" >
					<label>小图:</label>
					<input id="honorSmallImageUrl" name="honorSmallImageUrl" required="true" type="text" readonly="readonly" >
				</div>
				<div class="fitem">
