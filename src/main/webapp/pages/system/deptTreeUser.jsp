...
<script type="text/javascript">
	...
	$(function(){
		...
		//单击事件
		tree.setOnClickHandler(function(id,state){parent.mainFrame.document.location="${base}/system/SysUser/userListOrEdit/"+id;});
		//拖拽事件
		...
		tree.setXMLAutoLoading("${base}/system/SysUser/creatSysDeptOrUserTree");
		tree.setDataMode("json");
		tree.loadJSON("${base}/system/SysUser/creatSysDeptOrUserTree?id=" + (-1));
	});
	...
	*/
	function onDrag(sourceId, targetId, siblingId, sObject, tObject){
		...
		$.ajax({
			url : "${base}/system/SysUser/saveDragAndDrop",
			async : false,
			dataType : "json",
		...
		if(isContinue){
			parent.mainFrame.document.location="${base}/system/SysUser/userList/"+targetId;
			//window.location.href="${base}/system/SysUser/listPage/"+sourceId;
			return true;
		}else{
			return false;
		}
	...
	function RefreshDTreeInAdd(DeptId, nodeId, cdNm) {
		...
		if(state){
			tree.insertNewChild(DeptId, 0+nodeId, cdNm, 0,"people.png",0,0,"SELECT,CALL,CHECKED");
			parent.mainFrame.document.location="${base}/system/SysUser/userListOrEdit/" + DeptId;
		}else{
			var g=
			refreshItem(DeptId);
