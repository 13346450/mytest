...
(function($,undefined){
	$.fn.zyUpload02 = function(options,param){
		var otherArgs = Array.prototype.slice.call(arguments, 1);
		if (typeof options == 'string') {
			var fn = this[0][options];
			if($.isFunction(fn)){
				...
				throw ("zyUpload02 - No such method: " + options);
			}
		}
		...
		return this.each(function(){
			 ...
			 * 返回: 无
			 */
			this.funSetStatusInfo02 = function(files){
				var size = 0;
				var num = files.length;
				$.each(files, function(k,v){
					// 计算得到文件总大小
			...
			this.createCorePlug = function(){
				var params = {
					fileInput: $("#fileImagetwo").get(0),
					uploadInput: $("#fileSubmit").get(0),
					dragDrop: $("#fileDragArea02").get(0),
					url: $("#uploadForm02").attr("action"),
					...
					onSelect: function(selectFiles, allFiles) {
						para.onSelect(selectFiles, allFiles);  // 回调方法
						self.funSetStatusInfo02(ZYFILE02.funReturnNeedFiles());  // 显示统计信息
						var html = '', i = 0;
						// 组织预览html
						var funDealtPreviewHtml = function() {
							file = selectFiles[i];
						...
						var funBindDelEvent = function(){
							if($(".file_del").length>0){
								// 删除方法
								$(".file_del").click(function() {
									ZYFILE02.funDeleteFile(parseInt($(this).attr("data-index")), true);
									return false;	
								});
							}
							...
							if($(".file_edit").length>0){
								...
								$(".file_edit").click(function() {
									// 调用编辑操作
									//ZYFILE02.funEditFile(parseInt($(this).attr("data-index")), true);
									return false;	
								});
							}
						};
					...
					onSuccess: function(file, response) {
						...
						if(para.finishDel){
							...
							$("#uploadList_" + file.index).fadeOut();
							// 重新设置统计栏信息
							self.funSetStatusInfo02(ZYFILE02.funReturnNeedFiles());
						}
					},
					onFailure: function(file) {
						$("#uploadProgress_" + file.index).hide();
				...
				ZYFILE02 = $.extend(ZYFILE02, params);
				ZYFILE02.init();
			};
			...
			this.addEvent = function(){
				// 如果快捷添加文件按钮存在
				if($(".filePickertwo").length > 0){
					// 绑定选择事件
					$(".filePickertwo").bind("click", function(e){
		            	$("#fileImagetwo").click();
		            });
				}
				...
				$(".upload_btn").bind("click", function(e){
					// 判断当前是否有文件需要上传
					if(ZYFILE02.funReturnNeedFiles().length > 0){
						$("#fileSubmit").click();
					}else{
						alert("请先选中文件再点击上传");
					}
